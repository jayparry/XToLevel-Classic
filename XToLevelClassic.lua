...
local function updateText()
  ensureInit()
  local s = compute()
  local mode = XToLevelClassicDB.mode
  local label = (mode == "last") and "last" or "avg"
...
f:SetScript("OnEvent", function(self, ev, a1)
  ...
  if ev == "PLAYER_LOGIN" then ensureInit(); DEFAULT_CHAT_FRAME:AddMessage("XToLevelClassic loaded. Use /xtl"); updateText()
  elseif ev == "PLAYER_LEVEL_UP" then ensureInit(); XToLevelClassicDB.data.kills = {}; XToLevelClassicDB.data.quests = {}; updateText()
  elseif ev == "PLAYER_XP_UPDATE" then ensureInit(); updateText()
  elseif ev == "CHAT_MSG_SYSTEM" then ensureInit(); local xp = parseFirstNumber(a1 or ""); if xp then pushSample(XToLevelClassicDB.data.quests, xp, XToLevelClassicDB.lengths.quests); XToLevelClassicDB.data.xpSession = (XToLevelClassicDB.data.xpSession or 0) + xp; updateText(); dprint("Quest XP +"..xp) end
  elseif ev == "CHAT_MSG_COMBAT_XP_GAIN" then ensureInit(); local xp = parseFirstNumber(a1 or ""); if xp then pushSample(XToLevelClassicDB.data.kills, xp, XToLevelClassicDB.lengths.kills); XToLevelClassicDB.data.xpSession = (XToLevelClassicDB.data.xpSession or 0) + xp; updateText(); dprint("Kill XP +"..xp) end
  elseif ev == "PLAYER_ENTERING_WORLD" then ensureInit(); updateText()
  end
end)
...
SlashCmdList["XTOLEVEL"] = function(msg)
  ...
  if msg == "" or msg == "help" then
    ...
    DEFAULT_CHAT_FRAME:AddMessage("  /xtl len k <n>    - set kill window (for avg mode)")
    DEFAULT_CHAT_FRAME:AddMessage("  /xtl len q <n>    - set quest window (for avg mode)")
    ...
    DEFAULT_CHAT_FRAME:AddMessage("  /xtl reset        - reset data")
  ...
  elseif msg == "debug" then XToLevelClassicDB.debug = not XToLevelClassicDB.debug; DEFAULT_CHAT_FRAME:AddMessage("XToLevel: debug "..(XToLevelClassicDB.debug and "ON" or "OFF"))
    ...
    if m == "avg" or m == "last" then
      XToLevelClassicDB.mode = m; DEFAULT_CHAT_FRAME:AddMessage("XToLevel: mode set to "..m); updateText()
      return
    end
